...
def create_resume_command(decision: str, reviewer: str | None = None, comments: str | None = None) -> Command:
    ...
    """
    resume_data = {"decision": decision, "reviewer": reviewer, "comments": comments}

    client_logger.info(
        "Created resume command",
        extra={"decision": decision, "has_reviewer": reviewer is not None, "has_comments": comments is not None},
    )

    return Command(resume=resume_data)


def extract_interrupt_data(response: Any) -> dict[str, Any] | None:
...
def validate_resume_decision(decision: str, allowed_options: list | None = None) -> bool:
