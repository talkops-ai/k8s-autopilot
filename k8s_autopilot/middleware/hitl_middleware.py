...
try:
    ...
    HumanInTheLoopMiddleware = None  # type: ignore[assignment,misc]
    HITL_MIDDLEWARE_AVAILABLE = False
    hitl_logger.warning(
        "HumanInTheLoopMiddleware not available. Tool-level HITL will be disabled.",
...
def create_hitl_middleware_config(
    tools: dict[str, dict[str, Any]] | None = None, description_prefix: str = "Action pending review"
) -> Any | None:
    ...
    if not HITL_MIDDLEWARE_AVAILABLE or HumanInTheLoopMiddleware is None:
        hitl_logger.warning(
            "HumanInTheLoopMiddleware not available, returning None", extra={"fallback": "custom_gates_only"}
        )
        return None
    ...
    try:
        middleware = HumanInTheLoopMiddleware(
            interrupt_on=cast(dict[str, Any], interrupt_config), description_prefix=description_prefix
        )

        hitl_logger.info(
            "Created HITL middleware configuration",
            extra={"tool_count": len(interrupt_config), "tools": list(interrupt_config.keys())},
        )
        ...
        hitl_logger.error(
            f"Failed to create HITL middleware: {e}", extra={"error": str(e), "error_type": type(e).__name__}
        )
        return None
...
def create_supervisor_with_hitl(
    model: Any,
    tools: list[BaseTool],
    hitl_tools: dict[str, dict[str, Any]] | None = None,
    ...
    **agent_kwargs: Any,
    ...
    checkpointer: Any = agent_kwargs.pop("checkpointer", None)
    ...
    middleware_config = create_hitl_middleware_config(tools=hitl_tools, description_prefix=description_prefix)
    ...
    hitl_logger.info(
        "Creating supervisor agent with HITL middleware",
        extra={
    ...
    create_agent_fn: Any = create_agent

    # Create agent with middleware
    try:
        agent = create_agent_fn(
            model=model, tools=tools, middleware=middleware_list or [], checkpointer=checkpointer, **agent_kwargs
        )

        hitl_logger.info(
            "Successfully created supervisor agent with HITL",
            extra={"has_middleware": middleware_config is not None, "has_checkpointer": checkpointer is not None},
        )
        ...
        hitl_logger.error(
            f"Failed to create agent with HITL middleware: {e}",
            extra={"error": str(e), "error_type": type(e).__name__, "fallback": "creating_agent_without_middleware"},
        )
        # Fallback: create agent without middleware
        return create_agent_fn(model=model, tools=tools, checkpointer=checkpointer, **agent_kwargs)


def add_hitl_to_existing_agent(
    agent: Any, hitl_tools: dict[str, dict[str, Any]] | None = None, description_prefix: str = "Action pending review"
) -> Any:
