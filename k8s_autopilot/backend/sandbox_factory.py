...
def _create_langsmith_sandbox(config: SandboxConfig) -> Any:
    ...
    api_key = os.getenv("LANGSMITH_SANDBOX_API_KEY") or os.getenv("LANGSMITH_API_KEY") or os.getenv("LANGCHAIN_API_KEY")
    if not api_key:
...
def _create_other_sandbox(config: SandboxConfig) -> Any:
    ...
    if provider == "daytona":
        try:
            import daytona  # type: ignore[import-not-found,import-untyped]  # pyrefly: ignore[missing-import]  # Lazy import: optional dependency
            from langchain_daytona import (
                DaytonaSandbox,  # type: ignore[import-not-found,import-untyped]  # pyrefly: ignore[missing-import]; Lazy import: optional dependency
            )
        except ImportError as e:
        ...
        try:
            from langchain_modal import (
                ModalSandbox,  # type: ignore[import-not-found,import-untyped]  # pyrefly: ignore[missing-import]; Lazy import: optional dependency
            )
            import modal  # type: ignore[import-not-found,import-untyped]  # pyrefly: ignore[missing-import]  # Lazy import: optional dependency
        except ImportError as e:
        ...
        try:
            from langchain_runloop import (
                RunloopProvider,  # type: ignore[import-not-found,import-untyped]  # pyrefly: ignore[missing-import]; Lazy import: optional dependency
            )
        except ImportError as e:
...
def get_sandbox_backend(thread_id: str | None = None) -> Any:
    ...
    with _active_sandboxes_lock:
        ...
        backend = _create_langsmith_sandbox(config) if config.provider == "langsmith" else _create_other_sandbox(config)
        ...
        if backend and provider_info:
            ...
            try:
                if provider == "langsmith":
                    ...
                    from langchain_runloop import (  # type: ignore[import-not-found,import-untyped]  # pyrefly: ignore[missing-import]
                        RunloopProvider,
                    )
