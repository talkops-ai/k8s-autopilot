"""Marketplace parsing and materialization for K8s Autopilot."""
...
def get_marketplace_cache_dir() -> Path:
    """Return cache directory for marketplace catalogs."""
    base_dir = os.getenv("PLUGIN_CACHE_DIR")
    if base_dir:
...
def opaque_cache_key(value: str) -> str:
    """Return a sanitized, short SHA256 hex string for a cache key."""
...
def materialize_plugin_source(marketplace: PluginMarketplace, plugin: MarketplacePluginEntry) -> Path | None:
    ...
    if raw is not None:
        ...
        if isinstance(metadata_root, str) and raw.startswith("./"):
            clean_meta = metadata_root.removeprefix("./").strip("/")
            clean_raw = raw.removeprefix("./").strip("/")
