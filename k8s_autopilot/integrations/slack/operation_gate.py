...
If the operation is ``write`` and the target domain is **not** in the
...
class SlackOperationGate:
    ...
    async def _get_classifier(self) -> Any:
        ...
        if self._classifier is None:
            ...
            model_spec = getattr(self._config, "model_name", None) or "gemini-3.7-flash"
            model_res = create_model(model_spec)
            self._classifier = model_res.model.with_structured_output(
