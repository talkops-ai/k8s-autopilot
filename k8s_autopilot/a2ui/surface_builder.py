...
from k8s_autopilot.a2ui.risk_classification import RiskLevel
...
def build_hitl_approval_surface(
    ...
    if options is None:
        options = [
            {"id": "approve", "label": "1. Approve (y)"},
            {"id": "auto_approve_all", "label": "2. Enable Auto for this thread (a)"},
            {"id": "reject", "label": "3. Reject (n)"},
        ]
...
def build_ask_user_surface(
    ...
    title: str = "",
    action_id: str = "ask_user_response",
) -> list[dict[str, Any]]:
    """Build an ``askUserCard`` surface for interactive user questions.

    Emits the full lifecycle: createSurface → updateComponents → updateDataModel.

    Args:
        surface_id: Unique surface identifier (e.g., ``ask-user-a1b2c3d4``).
        questions: List of question dicts with ``question``, ``type`` (text/multiple_choice),
            ``choices``, and ``required``.
        title: Optional custom title for the card.
        action_id: Action name for submitting answers (defaults to ``ask_user_response``).

    Returns:
        List of A2UI operation dicts.
    """
    count = len(questions)
    default_title = "Agent has a question for you" if count <= 1 else f"Agent has {count} questions for you"
...
def update_ask_user_data(
    surface_id: str,
    **updates: Any,
) -> dict[str, Any]:
    """Build an ``updateDataModel`` operation for an existing askUserCard surface.

    Args:
        surface_id: The surface to update.
        **updates: Field-value pairs to update (e.g., ``status="answered"``).
...
def build_goal_confirmation_surface(
    ...
    action_id: str = "goal_response",
) -> list[dict[str, Any]]:
    """Build a ``goalConfirmationCard`` surface for goal criteria review.

    Emits the full lifecycle: createSurface → updateComponents → updateDataModel.

    Args:
        surface_id: Unique surface identifier (e.g., ``goal-confirm-a1b2c3d4``).
        goal_text: Description of the goal being reviewed.
        criteria: List of success criteria strings or checklists.
        status: Goal state (``in_progress`` | ``achieved`` | ``blocked``).
        action_id: Action name for decision submission (defaults to ``goal_response``).
...
def update_goal_confirmation_data(
    surface_id: str,
    **updates: Any,
) -> dict[str, Any]:
