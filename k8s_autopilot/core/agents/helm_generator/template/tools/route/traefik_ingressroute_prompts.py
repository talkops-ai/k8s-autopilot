...
### 5. HELM TEMPLATING (CRITICAL SYNTAX RULES)

**ALWAYS use DOUBLE QUOTES** in Go templates - NEVER single quotes
**REPLACE CHARTNAME** with the actual chart name provided

- **Namespace**: ALWAYS use `{{ .Values.namespace.name | default .Release.Namespace }}`
- **Service Names**: ALWAYS use `{{ include "CHARTNAME.fullname" . }}`
- **Service Ports**: ALWAYS use `{{ .Values.service.port }}` or `{{ .Values.service.targetPort }}`
- **Other Values**: Use `{{ .Values.* }}` for ALL configurable parameters

**Standard Helm Functions:**
- Use `{{ include "CHARTNAME.labels" . | nindent 4 }}` for labels
- Use `{{ include "CHARTNAME.fullname" . }}` for resource names and service references
- Use `{{ .Release.Name }}`, `{{ .Release.Namespace }}` for dynamic values
- Use `{{- if .Values.* }}` for conditional sections

**IMPORTANT**: The service name in `spec.routes[].services[].name` MUST match the Service resource name generated by the same Helm chart. Use `{{ include "CHARTNAME.fullname" . }}` to ensure consistency.
...
**Namespace (for context only, use Helm templating):** {namespace}

**IMPORTANT REMINDER:**
- For namespace in YAML, use: `{{{{ .Values.namespace.name | default .Release.Namespace }}}}`
- For service name in YAML, use: `{{{{- include "{app_name}.fullname" . }}}}`
- For service port in YAML, use: `{{{{ .Values.service.port }}}}`
...
### Services (Reference Only - Use Helm Templating!)
{Services_description}

**NOTE:** The service name above is for context. In the generated YAML, use `{{{{- include "{app_name}.fullname" . }}}}` instead of hardcoding.
...
3. **Service Name** must match the Service resource name exactly
