...
class MCPDiscoveryMiddleware(BaseAgentMiddleware):
    ...
    def discover_and_bind(self) -> List[Dict[str, Any]]:
        ...
        if self._project_root:
            ...
            raw_root = getattr(cfg, "AGENT_PROJECT_ROOT", None) or ""
            root = Path(raw_root).resolve() if raw_root.strip() else Path.cwd()

        # Step 1: Discover project-level servers
        trust = MCPTrustStore()
        discovery = MCPDiscovery(trust_store=trust)
        discovered = discovery.discover(project_root=root)

        # Step 2: Merge with defaults
        default_servers = cfg.mcp_config.get("servers", [])
        merged = discovery.merge_with_defaults(default_servers, discovered)

        self._discovered_servers = merged

        if discovered:
            logger.info(f"MCP Discovery: {len(discovered):d} project servers found, {len(merged):d} after merge")
