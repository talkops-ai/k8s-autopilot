...
from k8s_autopilot.core.integration.identity import IdentityMapper
from k8s_autopilot.core.integration.stream_bridge import GraphStreamBridge
...
class SlackMessagingIntegration(MessagingIntegration):
    ...
    This class is the central coordinator for Slack integration.  It
    implements the abstract :class:`MessagingIntegration` interface and
    composes the generic building blocks (:class:`GraphStreamBridge`,
    :class:`ThreadStore`, :class:`IdentityMapper`) with the
    Slack-specific :class:`SlackStreamSink`.

    Usage (via Bolt handlers — see ``bolt_handlers.py``)::

        integration = SlackMessagingIntegration(config, bolt_app, graph)
        # ...register_bolt_handlers(bolt_app, integration)
        # Bolt event → integration.handle_message(IncomingMessage)
    """
