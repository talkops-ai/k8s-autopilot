...
from slack_bolt.async_app import AsyncApp, AsyncAssistant, AsyncSetStatus, AsyncSetSuggestedPrompts, AsyncSay  # type: ignore[import-not-found]
...
if TYPE_CHECKING:
    from k8s_autopilot.core.integration.slack.integration import (
...
def register_bolt_handlers(
    ...
    async def handle_thread_started(
        ...
        """When user opens the assistant container, set suggested prompts.

        Per Slack Bolt docs, ``AsyncAssistant.thread_started`` is the
        recommended way to handle ``assistant_thread_started`` events.
        The framework injects typed utilities (``AsyncSay``,
        ``AsyncSetSuggestedPrompts``) automatically.
        """
            ...
            prompts=[
                {
                    ...
                    "message": "Show me the health status of all clusters",
                },
                {
                    "title": "Deploy Service",
                    "message": "Deploy my-service to staging namespace",
                },
                {
                    "title": "Pod Issues",
                    "message": "What pods are in CrashLoopBackOff?",
                },
                {
                    "title": "Resource Usage",
                    "message": "Show CPU and memory usage for production",
                },
            ],
        )

        logger.info("Set suggested prompts for new assistant thread")
