"""
Eval: Observability Operator YAML scenario dataset tests.

Runs each scenario from ``tests/evals/dataset/observability_operator/*.yaml``
through the ObservabilityCoordinator with a scripted fake model, then evaluates
the trajectory using all 7 rule-based evaluators.
...
- The evaluator pipeline covers ALL expectation fields in the YAML schema:
...
To run with a real LLM (for trajectory quality evaluation):
    EVAL_USE_REAL_LLM=1 pytest tests/evals/test_observability_eval_scenarios.py -v
"""
import os
import pytest
...
from tests.evals.observability_operator.runner import load_observability_dataset, run_observability_scenario
...
def _make_scenario_fake_model(scenario: dict) -> FakeMessagesListChatModel:
    ...
    primary_subagent = must_call[0] if must_call else "prometheus-operator"
    return FakeMessagesListChatModel(responses=[
        AIMessage(
            content="",
            tool_calls=[{
...
_SCENARIOS = load_observability_dataset()
...
async def test_observability_eval_scenario(scenario):
    """
    ...
    trace = await run_observability_scenario(scenario, fake_model=fake_model)

    if trace.get("skipped"):
        pytest.skip(trace.get("skip_reason", "skipped"))

    # Run all 7 evaluators — collect failures for a comprehensive report
    failures = []
