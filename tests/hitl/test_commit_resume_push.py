...
@pytest.mark.hitl
@pytest.mark.asyncio
async def test_commit_resume_push_proceeds_to_github(mock_config, in_memory_store, memory_saver):
    ...
    fake_llm = BindableFakeModel(responses=[
        ...
        AIMessage(content="", tool_calls=[{"name": "github-agent", "args": {}, "id": "tc4"}]),
    ...
    with patch("k8s_autopilot.core.agents.helm_operator.coordinator.create_model", return_value=fake_llm):
        ...
        config = {"configurable": {"thread_id": "hitl-push-001"}}
    ...
    if snapshot.next:
        # Resume with 'push'
        resume_data = "push" # Or {"action": "push"} depending on A2UI schema
        try:
            await agent.ainvoke(Command(resume=resume_data), config=config)
        except Exception as e:
            # Maybe another interrupt or end
            pass
            
        final_snapshot = agent.get_state(config)
        # Should proceed past the gate
        assert not final_snapshot.next or final_snapshot.next != snapshot.next
